                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <h1 className="flex flex-col leading-none">
                                <span className="text-[14px] font-bold text-indigo-600 uppercase tracking-widest">Print</span>
                                <span className="text-xl font-bold tracking-tight text-black/80 mt-1">Mfg</span>
                            </h1>
                        ...
                        <div className="flex items-center gap-2">
                            {['All', 'Cartons', 'Inserts', 'Labels'].map(cat => (
                                <button
                                    ...
                                    className={`
                                    px-5 py-2 rounded-full text-[12px] font-bold uppercase tracking-wider transition-all border
                ...
                {PROCESS_STEPS.map((step, idx) => {
                    ...
                    return (
                        <section
                            ...
                            `}
                        >
                            {/* Process Header - Even More Compact */}
                            <div className="px-4 py-1 border-b border-slate-300 flex items-center justify-between bg-slate-100">
                                <div className="flex items-center gap-2">
                                    <div className={`w-2 h-2 rounded-full ${step === 'Ready' ? 'bg-emerald-500' :
                                        step === 'Hold' ? 'bg-rose-500' : 'bg-indigo-500'
                                        }`} />
                                    <h3 className="font-bold text-black/80 text-[12px] uppercase tracking-[0.1em]">{step}</h3>
                                </div>
                                <span className="text-black/80 text-[11px] font-bold">
                                    {stepOrders.length}
                                </span>
                            </div>

                            {/* Jobs Container - Vertical Stack */}
                            <div className="p-1 min-h-[30px] flex flex-col gap-1">
                                {stepOrders.length === 0 ? (
                                    <div className="w-full py-1 text-center text-slate-300 text-[8px] font-normal uppercase tracking-[0.1em]">
                                        -
                                    </div>
                                ) : (
                                    stepOrders.map(order => (
                                        <div
                                            ...
                                            className={`
                                                flex items-center gap-2 px-2 py-1 rounded-lg transition-all cursor-grab active:cursor-grabbing group relative
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0 flex items-center justify-between gap-3">
                                                {/* Left Section: Product Name & Specs on same line */}
                                                ...
                                                {/* Middle Section: Stats - More Compact */}
                                                <div className="flex items-center gap-6 shrink-0">
                                                    <div className="flex items-center gap-2 border-r border-black/10 pr-6">
                                                        <span className="text-black/50 uppercase text-[9px] font-bold tracking-widest">ORDER</span>
                                                        <span className="text-black/80 font-bold text-[13px]">{(order.quantity || 0).toLocaleString()}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-black/50 uppercase text-[9px] font-bold tracking-widest">PRINT</span>
                                                        <span className="text-blue-600 font-bold text-[13px]">{(order.total_print_qty || 0).toLocaleString()}</span>
                                                    </div>
                                                </div>
