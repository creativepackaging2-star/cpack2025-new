...
import { Loader2, ChevronLeft, Palette, MessageCircle, User } from 'lucide-react';
...
function PunchingSummaryContent() {
    ...
    const sendWhatsApp = () => {
        ...
        const phone = selectedPrinter.phone.replace(/\D/g, '');
    ...
    return (
        <div className="p-4 md:p-8 max-w-[1400px] mx-auto">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4 print:hidden">
                ...
                <div className="flex flex-wrap items-center gap-3 bg-white p-2 border border-slate-200 rounded-xl shadow-sm">
                    <div className="flex items-center gap-2 px-3 border-r border-slate-200">
                        ...
                        <select
                            className="text-xs font-bold text-slate-700 outline-none bg-transparent min-w-[150px]"
                            onChange={(e) => {
                    ...
                    <button
                        ...
                        className="bg-emerald-600 text-white px-4 py-2 rounded-lg font-black text-xs hover:bg-emerald-700 transition-all shadow-md flex items-center gap-2 active:scale-95"
                    >
                    ...
                    <button
                        ...
                        className="bg-rose-600 text-white px-4 py-2 rounded-lg font-black text-xs hover:bg-rose-700 transition-all shadow-md flex items-center gap-2 active:scale-95"
                    >
                        Print Summary
                    </button>
                </div>
            ...
            <div className="overflow-x-auto bg-white rounded-2xl border-2 border-slate-200 shadow-xl overflow-hidden">
                <table className="w-full border-collapse text-left">
                    <thead>
                        <tr className="bg-slate-900 text-white">
                            <th className="px-4 py-4 border-r border-slate-700 text-[10px] font-black uppercase tracking-wider w-[55px] text-center">Sr. No.</th>
                            <th className="px-6 py-4 border-r border-slate-700 text-[10px] font-black uppercase tracking-wider min-w-[250px]">Product & Artwork</th>
                            <th className="px-4 py-4 border-r border-slate-700 text-[10px] font-black uppercase tracking-wider text-center">Print Size</th>
                            <th className="px-4 py-4 border-r border-slate-700 text-[10px] font-black uppercase tracking-wider text-center">Print Qty</th>
                            <th className="px-4 py-4 border-r border-slate-700 text-[10px] font-black uppercase tracking-wider text-center">Embossing</th>
                            <th className="px-4 py-4 border-r border-slate-700 text-[10px] font-black uppercase tracking-wider text-center">Pasting</th>
                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-center bg-rose-950/20">Max Del. Qty</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                        {orders.map((order, index) => (
                            <tr key={order.id} className="hover:bg-slate-50/80 transition-colors group">
                                <td className="px-4 py-4 border-r border-slate-100 text-xs font-bold text-slate-400 text-center">{index + 1}</td>
                                <td className="px-6 py-4 border-r border-slate-100">
                                    <div className="text-sm font-black text-slate-800 group-hover:text-rose-600 transition-colors uppercase tracking-tight">{order.products?.product_name || order.product_name}</div>
                                    <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">{order.products?.artwork_code || order.artwork_code || '-'}</div>
                                </td>
                                <td className="px-4 py-4 border-r border-slate-100 text-[13px] font-bold text-slate-700 text-center">{order.print_size || '-'}</td>
                                <td className="px-4 py-4 border-r border-slate-100 text-sm font-black text-slate-900 text-center">{(order.total_print_qty || 0).toLocaleString()}</td>
                                <td className="px-4 py-4 border-r border-slate-100 text-sm font-black text-center">
                                ...
                                <td className="px-4 py-4 border-r border-slate-100 text-sm font-bold text-slate-700 text-center">{order.pasting_type || '-'}</td>
                                <td className="px-6 py-4 text-base font-black text-rose-600 text-center bg-rose-50/30">
