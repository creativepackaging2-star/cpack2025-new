...
export default function ProductionBoardGrid() {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 font-sans selection:bg-indigo-100 text-slate-900 overflow-hidden">
            {/* Darker Page Header */}
            <header className="px-4 py-3 border-b border-slate-200 bg-slate-50 shrink-0">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-indigo-50 rounded-lg shadow-sm border border-indigo-100">
                            <Workflow className="w-5 h-5 text-indigo-600" />
                        </div>
                        <h1 className="text-xl font-bold tracking-tight text-slate-900 uppercase">Production <span className="text-indigo-600">Board</span></h1>
                    </div>

                    <div className="flex items-center gap-1.5 p-1 bg-white shadow-sm border border-slate-200 rounded-lg">
                        {['All', 'Cartons', 'Inserts', 'Labels'].map(cat => (
                            <button
                                key={cat}
                                onClick={() => setSelectedCategory(cat)}
                                className={`
                                    px-4 py-2 rounded-md text-[13px] font-bold uppercase tracking-wider transition-all
