-- COMPREHENSIVE AUTOMATION: Snapshot EVERYTHING to Orders
-- This ensures the Order table holds all data even if the product/lookup tables change
...
DECLARE
    p RECORD;
BEGIN
    -- Fetch the product with all its lookup names
    SELECT 
        products.*,
        ...
        delivery_addresses.address as delivery_address_joined
    INTO p
    FROM products
    ...
    IF FOUND THEN
        ...
        -- Technical Snapshot (The Core Task)
        ...
        -- Lookup Names (The "Hard" snapshots)
...
COMMENT ON FUNCTION snapshot_product_to_order() IS 'Ensures ALL product data is snapshotted into orders table for historical integrity';
